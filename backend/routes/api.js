...
router.post('/scrape-web-url', azureAuth.authenticate, async (req, res) => {
    ...
    try {
        ...
        // Use environment variables for the function URL and key to avoid committing secrets
        const SCRAPE_FUNCTION_URL = process.env.AZURE_SCRAPE_FUNCTION_URL || 'https://smartdoc-ingestion.azurewebsites.net/api/scrape';
        const SCRAPE_FUNCTION_KEY = process.env.AZURE_SCRAPE_FUNCTION_KEY;

        if (!SCRAPE_FUNCTION_KEY) {
            console.error('Missing AZURE_SCRAPE_FUNCTION_KEY environment variable. Cannot call scrape function.');
            return res.status(500).json({ success: false, message: 'Server configuration error: missing scrape function key' });
        }

        const endpoint = `${SCRAPE_FUNCTION_URL}?code=${SCRAPE_FUNCTION_KEY}`;
