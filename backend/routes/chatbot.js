...
router.post('/my-chat-bot', async (req, res) => {
  try {
    ...
    // Use environment variables for the external chatbot function URL and key to avoid committing secrets
    const CHATBOT_FUNCTION_URL = process.env.CHATBOT_FUNCTION_URL || 'https://smartdocs-docxingestion.azurewebsites.net/api/chatbot';
    const CHATBOT_FUNCTION_KEY = process.env.CHATBOT_FUNCTION_KEY;

    if (!CHATBOT_FUNCTION_KEY) {
      console.error('Missing CHATBOT_FUNCTION_KEY environment variable. Cannot call external chatbot function.');
      return res.status(500).json({ success: false, error: 'Server configuration error: missing chatbot function key' });
    }

    const response = await axios.post(`${CHATBOT_FUNCTION_URL}?code=${CHATBOT_FUNCTION_KEY}`, {
      message,
      workspace_id,
      document_category,
      conversation_history,
